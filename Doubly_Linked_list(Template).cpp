...
public:
    ...
    Node(T val, Node<T>* ptr = NULL, Node<T>* pre = NULL)
    {
        element = val;
        nextNode = ptr;
        prevNode = pre;
...
public:
    ...
    int erase(T value)
    {
        ...
        for (Node<T>* ptr = head; head != NULL && head->get_element() == value; ptr = head)
        {
            Node<T>* temp = head;
            head = head->get_next_node();
            if (head != NULL) head->set_prev_node(NULL);
            temp->set_next_node(NULL);
            delete temp;
            count++;
        }
        ...
        for (Node<T>* ptr = head->get_next_node(); ptr != NULL; ptr = ptr->get_next_node())
        {
            if (ptr->get_element() == value)
            {
                ...
                if (ptr->get_next_node() != NULL)
                {
                    ptr->get_next_node()->set_prev_node(prev);
                }
                Node<T>* temp = ptr;
                ptr = prev;
                temp->set_next_node(NULL);
